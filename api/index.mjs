...
import startHandler from '../apps/web/dist/server/server.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const clientRoot = path.resolve(__dirname, '../apps/web/dist/client');
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.error('[vercel-web] request handling failed', error);
    res.statusCode = 500;
    res.setHeader('content-type', 'application/json');
