...
- **内容列是软墙**：走线主要在两侧空白里长；到达列边缘时按 `ENTER_COLUMN` 的概率进入，进入后
  不分叉、比两侧更早结束（`SURVIVE_IN_COLUMN`），mask 在 `MASK_FADE` 宽度内从 `MASK_EDGE`
  渐变到 `MASK_COLUMN`。目的：两侧有连接感、中间也有淡淡的延伸，但不抢正文。首页的卡片和
  文章面板是不透明的，会盖住背后的走线，所以中间可见的部分主要在字符画周围和面板下方。
  点阵波面整屏铺开，靠中心压暗（`DIM_*`）。
